...
class TCTS():
    ...
    def fit(
        ...
        # )
        # 需要自己准备数据
        df_train = dataset.iloc[:int(len(dataset)*0.7), :]
        df_valid = dataset.iloc[int(len(dataset)*0.7):int(len(dataset)*0.9), :]
        df_test = dataset.iloc[int(len(dataset)*0.9):, :]
        if df_train.empty or df_valid.empty:
            raise ValueError("Empty data from dataset, please check your dataset config.")

        # 取df_train的feature_0和feature_1作为训练数据
        x_train, y_train = df_train[['feature_0', 'feature_1']], df_train['label']
        x_valid, y_valid = df_valid[['feature_0', 'feature_1']], df_valid['label']
        x_test, y_test = df_test[['feature_0', 'feature_1']], df_test['label']
...
if __name__ == '__main__':
    # 随机构造一个pandas.DataFrame
    df = pd.DataFrame(np.random.rand(1000, 3))
    df.columns = ['feature_' + str(i) for i in range(2)] + ['label']
    # 将df划分为训练集和测试集
    

    tcts = TCTS()
    tcts.fit(dataset=df, save_path='tcts')
    print("Done!")