...
if __name__ == '__main__':
    ...
    for year in range(TrainConfig.train_begin_year, TrainConfig.train_begin_year + TrainConfig.rolling_window):
        ...
        if MODEL == "MLP":
            model = MLPModel(ModelConfig.input_size, 1)
        elif MODEL == "GRU":
        ...
        for epoch in range(TrainConfig.num_epochs):
            ...
            for batch_x, batch_y in train_progress_bar:
                ...
                train_progress_bar.set_description(f"Epoch {epoch + 1} train. Training Loss: {training_loss / len(train_loader):.6f}")
            ...
            if TrainConfig.valid_time_span > 0:
                ...
                with torch.no_grad():
                    ...
                    for batch_x, batch_y in valid_progress_bar:
                        ...
                        valid_progress_bar.set_description(f"Epoch {epoch + 1} validation. Validation Loss: {valid_loss / len(valid_X) * TrainConfig.batch_size:.6f}")
            scheduler.step()
